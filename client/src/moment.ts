...
export const momentTest = async () => {
  ...
  let expiredTime = await moment.utc(
    localStorage.getItem('accessTokenExpireTime'),
  );
  let curTime = await moment.utc();
  let diffTime: any = await moment
    .duration(curTime.diff(expiredTime))
    .asMilliseconds();
    ...
    await axios
      ...
      .then((res) => {
        ...
        localStorage.setItem(
          ...
          `${res.data.accessTokenExpireTime}`,
        );
        axios.defaults.headers.common['Authorization'] =
...
axios.interceptors.request.use((config) => {
  ...
  if (
    config.url ===
    process.env.REACT_APP_DB_HOST + '/api/access-token/issue'
  ) {
